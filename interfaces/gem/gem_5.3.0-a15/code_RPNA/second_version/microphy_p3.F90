...
!==================================================================================================!
 ...
 subroutine p3_init(lookup_file_dir,nCat,trplMomI,liqfrac,p3_iparam,model,stat,abort_on_err,dowr) ! FRED Add iparam
...
! Passed arguments:
 ...
 integer,          intent(in)             :: p3_iparam          ! 1:Seifert & Beheng 2000, 2:Beheng 1994, 3: K&K 2000 (default), 4: Kogan 2013 ! FRED Add iparam
...
! = 4 Kogan 2013
 iparam = p3_iparam ! FRED Add p3_iparam
...
! New process rates with log_LiquidFrac (present(qiliq))
 ...
 real    :: lammax,lammin,mu,dv,sc,dqsdT,ab,kap,epsr,epsc,xx,aaa,epsilon,sigvl,epsi_tot, &
            ...
            deltaD_init,dum1c,dum4c,dum5c,dumt,qcon_satadj,qevp_satadj,qdep_satadj,sources,sinks,    & ! FRED DEBUG SCPF
...
! This change is tiny and therefore neglected. But beware of long time step!
!      if ((t(i,k).lt.273.15 .and. supi(i,k).ge.-0.05) .or.                              & ! FRED DEBUG obsolete: trust SCPF
!          (t(i,k).ge.273.15 .and. sup(i,k).ge.-0.05 ) .and. (.not. SCPF_on))            & ! FRED DEBUG obsolete: trust SCPF
!          log_nucleationPossible = .true. ! FRED DEBUG obsolete: trust SCPF
   ...
   !first call to compute_SCPF
    call compute_SCPF(Qc(i,:)+sum(Qitot(i,:,:),dim=2),Qr(i,:),Qv_old(i,:),Qvi(i,:),          & ! FRED DEBUG changed Qv-->Qv_old
   ...
   !if ((SCPF_on) .and. (sum(SCF) .ge. 0.01)) log_nucleationPossible = .true.
    if (                 sum(SCF) .ge. 0.01 ) log_nucleationPossible = .true. ! FRED DEBUG: SCPF always ON now
...
!   main k-loop (for processes):
    k_loop_main: do k = kbot,ktop,kdir
     ...
     ! if relatively dry and no hydrometeors at this level, skip to end of k-loop (i.e. skip this level)
      ...
      !if ( (     SCPF_on) .and. log_exitlevel .and.       & ! FRED DEBUG SCPF always ON now
       if ( log_exitlevel .and. (SCF(k).lt.0.01) )  goto 555 !i.e. skip all process rates !%%% FRED TEST NOT SURE
 !     if ( (.not.SCPF_on) .and. log_exitlevel .and.       & ! FRED DEBUG obsolete: trust in SCPF
 !        ((t(i,k).lt.273.15 .and. supi(i,k).lt.-0.05) .or.& ! FRED DEBUG obsolete: trust in SCPF
 !         (t(i,k).ge.273.15 .and. sup(i,k) .lt.-0.05))) goto 555   !i.e. skip all process rates ! FRED DEBUG obsolete: trust in SCPF
...
! Note (BUG): is *iSPF(k) necessary here (epsr is in-precip anyway)
       ...
       if (qr(i,k)*iSPF(k).ge.qsmall) then ! FRED DEBUG : not sure *iSCPF necessary here for consistency reason...
...
!       dum = qvs(i,k)*rho(i,k)*g*uzpl(i,k)/max(1.e-3,(pres(i,k)-polysvp1(t(i,k),0)))
       ...
       !if (log_LiquidFrac) then
       ! FRED DEBUG : could (should?) add pressure change effect below :
         aaa = (qv(i,k)-qv_old(i,k))*odt - dqsdT*(-dum*g*inv_cp)-(qvs(i,k)-dumqvi)*(1.+xxls(i,k)*   &
               inv_cp*dqsdT)*oabi*epsi_tot ! FRED Note : can save computation here (dum redundancy)
       !else
       !  if (t(i,k).lt.273.15) then
...
!      if (.not. scpf_ON)  then ! FRED DEBUG obsolete: trust in SCPF
!         ssat_cld = ssat(i,k)  ! FRED DEBUG obsolete: trust in SCPF
!         ssat_r   = ssat(i,k)  ! FRED DEBUG obsolete: trust in SCPF
!         sup_cld  = sup(i,k)   ! FRED DEBUG obsolete: trust in SCPF
!         sup_r    = sup(i,k)   ! FRED DEBUG obsolete: trust in SCPF
!         supi_cld = supi(i,k)  ! FRED DEBUG obsolete: trust in SCPF
!      else                     ! FRED DEBUG obsolete: trust in SCPF
...
!      endif                    ! FRED DEBUG obsolete: trust in SCPF
...
! this is not applied at the first time step, since saturation adjustment is applied at the first step
       ...
       if (.not.(log_predictNc).and.sup_cld.gt.1.e-6.and.it.gt.1) then
          ...
          dumqvs = qv_sat(t(i,k),pres(i,k),0) ! FRED note: est-ce vraiment coherent?
          ...
          if (iparam.eq.1) then
            ...
            !Seifert and Beheng (2001)
             ...
             qcaut =  kc*1.9230769e-5*(nu(i,k)+2.)*(nu(i,k)+4.)/(nu(i,k)+1.)**2*         &
                   ...
                   ncautc = qcaut*7.6923076e+9 ! FRED NOTE: where does it come from? 

          elseif (iparam.eq.2) then

            !Beheng (1994)
!Note (BUG), needs to be in-cloud condition
             dum5 = nc(i,k)*iSCF(k)*rho(i,k)*1.e-6 ! FRED NOTE: could save a lot of time
             dum6 = qc(i,k)*iSCF(k)*rho(i,k)*1.e-3 ! FRED NOTE: with these dum5 and dum6...
             if (dum5 .gt. 1.) then ! FRED DEBUG must be gt 1/cm3
              if (dum5 .lt. 100.) then ! FRED DEBUG
                qcaut = 6.e+28*inv_rho(i,k)*mu_c(i,k)**(-1.7)*dum5**(-3.3)*dum6**(4.7)*SCF(k) ! FRED DEBUG
             else
               !2D interpolation of tabled logarithmic values
                dum   = 41.46 + (dum5-100.)*(37.53-41.46)*5.e-3 ! FRED DEBUG
                dum1  = 39.36 + (dum5-100.)*(30.72-39.36)*5.e-3 ! FRED DEBUG
                qcaut = dum+(mu_c(i,k)-5.)*(dum1-dum)*0.1
              ! 1000/rho is for conversion from g cm-3/s to kg/kg
                qcaut = exp(qcaut)*dum6**4.7*1000.*inv_rho(i,k)*SCF(k) ! FRED DEBUG
...
!                       inv_rho(i,k)*SCF(k)
             ...
             ncautc = 7.7e+9*qcaut ! FRED NOTE: equivalent to around 32.4 microns for the new drops
          ...
          elseif (iparam.eq.3) then
           ...
           !Khairoutdinov and Kogan (2000)
            ...
            ! note: ncautr is change in Nr; ncautc is change in Nc
             ncautr = qcaut*cons3           ! FRED NOTE: assuming drizzle drop radius = 25 microns
             ncautc = qcaut*nc(i,k)/qc(i,k) ! FRED NOTE: x% of qc converted is x% of nc converted !!?
          ...
          elseif (iparam.eq.4) then
           ...
           !Kogan (2013)
             ...
             ncautr = qcaut*cons8           ! FRED NOTE: assuming drizzle drop radius = 40 microns
             ncautc = qcaut*nc(i,k)/qc(i,k) ! FRED NOTE: x% of qc converted is x% of nc converted !!?
       ...
       if (qr(i,k).ge.qsmall .and. qc(i,k)*iSCF(k).ge.qsmall) then
          ...
          if (iparam.eq.1) then
           !Seifert and Beheng (2001)
             ...
             dum   = 1.-qc(i,k)*iSCF(k)/(qc(i,k)*iSCF(k)+qr(i,k)*iSPF(k)) ! FRED on peut sauver du calcul ici...
...
! be greater than the sum of the sources, thereby resulting in overdepletion.
    ...
    !Limit total condensation (incl. activation) and evaporation to saturation adjustment
       ...
       qevp_satadj  =((Qv_cld(k)-dumqvs)*(SPF(k)-SPF_clr(k))+(Qv_clr(k)-dumqvs)*SPF_clr(k))/(1.+xxlv(i,k)**2*dumqvs/(cp*rv*t(i,k)**2))*odt ! FRED DEBUG pas satisfaisant
       !qcon_satadj  = (Qv(i,k)-dumqvs)/(1.+xxlv(i,k)**2*dumqvs/(cp*rv*t(i,k)**2))*odt ! FRED ATTENTION BUG CONCEPTUEL

       tmp1 = qccon+qrcon+qcnuc+sum(qlcon)
       if (tmp1>0. .and. qcon_satadj<0.) then ! FRED DEBUG : devrait jamais arriver
          qccon = 0.
          qrcon = 0.
          qcnuc = 0.
          qlcon = 0.
          ncnuc = 0.
       elseif (tmp1.gt.0. .and. tmp1.gt.qcon_satadj) then ! FRED DEBUG
       ...
       tmp2 = qcevp+qrevp+sum(qlevp) ! FRED DEBUG
         !elseif (qcevp+qrevp+sum(qlevp).gt.0.) then ! FRED DEBUG
       if (tmp2>0. .and. qevp_satadj>0.) then ! FRED DEBUG : devrait jamais arriver
          qcevp = 0. ! FRED DEBUG
          qrevp = 0. ! FRED DEBUG
          nrevp = 0. ! FRED DEBUG
          qlevp = 0. ! FRED DEBUG
          nlevp = 0. ! FRED DEBUG
       elseif (tmp2.gt.0. .and. tmp2.gt.-qevp_satadj) then ! FRED DEBUG : devrait jamais arriver
    ...
    !Limit total deposition (incl. nucleation) and sublimation to saturation adjustment
       ...
       qdep_satadj = (qv_tmp-dumqvi)/(1.+xxls(i,k)**2*dumqvi/(cp*rv*t_tmp**2))*odt*SCF(k) ! FRED Note: Obtenir concensus ici
       ...
       if (tmp1>0. .and. qdep_satadj<0.) then
          ...
          do iice = 1,nCat
             ...
             nisub(iice)  = nisub(iice)*min(1.,qisub(iice)/dum) ! FRED Note: kind of heterogeneous mixing... good enough
          enddo
...
! cloud
       ...
       if (sinks.gt.sources .and. sinks.ge.1.e-20) then
         ...
         !       (4 multiplications, even if values are not used [if log_predictNc=.false.], are cheaper than one IF)
            ncautc = ncautc*ratio
            ncacc  = ncacc*ratio
            nccol  = nccol*ratio ! FRED Note: quid de ncevp ?
            ncheti = ncheti*ratio
           !nchetc = nchetc*ratio
            nccoll = nccoll*ratio
         !endif
...
!---------------------------------------------------------------------------------
   ...
   !-- ice-phase dependent processes:
       ...
       iice_loop3: do iice = 1,nCat
          ...
          if (log_predictNc) then
             nc(i,k) = nc(i,k) + (-nccol(iice)-nchetc(iice)-ncheti(iice)-nccoll(iice))*dt ! FRED Note : quid de ncevp?
          endif
   ...
   !-- warm-phase only processes:
       ...
       if (log_predictNc) then
          nc(i,k) = nc(i,k) + (-ncacc-ncautc+ncslf+ncnuc)*dt ! FRED Note: quid de ncevp ?
       else
          nc(i,k) = nccnst*inv_rho(i,k)
       endif
       ...
       substep_sedi_r: do while (dt_left.gt.1.e-4)
          ...
          kloop_sedi_r1: do k = k_qxtop,k_qxbot,-kdir
             ...
             qr_not_small_1: if (qr(i,k)*iSPF(k).ge.qsmall) then
               ...
               !Compute Vq, Vn:
                nr(i,k)  = max(nr(i,k),nsmall) ! FRED DEBUG excessive size sorting here also
             ...
             substep_sedi_i1: do while (dt_left.gt.1.e-4)
                ...
                kloop_sedi_i1: do k = k_qxtop,k_qxbot,-kdir
                   ...
                   qi_notsmall_i1: if (qitot(i,k,iice).ge.qsmall) then
                    ...
                    !--Compute Vq, Vn:
                      nitot(i,k,iice) = max(nitot(i,k,iice),nsmall) !impose lower limits to prevent log(<0) ! FRED DEBUG excessive size sorting here also
                      call calc_bulkRhoRime(qitot(i,k,iice),qirim(i,k,iice),qiliq(i,k,iice),birim(i,k,iice),rhop)
...
! homogeneous freezing of cloud and rain
    ...
    ! compute mean-mass ice diameters
       ...
       do iice = 1,nCat
          if (qitot(i,k,iice).ge.qsmall) then
             nitot(i,k,iice) = max(nitot(i,k,iice),nsmall) ! FRED DEBUG excessive size sorting here also
             call calc_bulkRhoRime(qitot(i,k,iice),qirim(i,k,iice),qiliq(i,k,iice),birim(i,k,iice),rhop)
             call find_lookupTable_indices_1a(dumi,dumjj,dumii,dumll,dum1,dum4,dum5,dum7,isize,         &
                        rimsize,liqsize,densize,qitot(i,k,iice),nitot(i,k,iice),qirim(i,k,iice),        &
                        qiliq(i,k,iice),rhop)
       ...
       qc_not_small_2: if (qc(i,k)*iSCF(k).ge.qsmall .and. t(i,k).lt.233.15) then
          ...
          Q_nuc = qc(i,k)
          nc(i,k) = max(nc(i,k),nsmall) ! FRED DEBUG excessive size sorting here also
          N_nuc = nc(i,k)
...
! and compute diagnostic fields for output
    ...
    ! ice:
       ...
       iice_loop_final_diagnostics:  do iice = 1,nCat
          ...
          ! impose mean ice size bounds (i.e. apply lambda limiters)
             nitot(i,k,iice) = min(nitot(i,k,iice),f1pr09*qitot(i,k,iice)) ! FRED DEBUG excessive size sorting here also
             nitot(i,k,iice) = max(nitot(i,k,iice),f1pr10*qitot(i,k,iice)) ! FRED DEBUG excessive size sorting here also
    ...
    i_loop_typediag: do i = its,ite
      ...
      !-- rain vs. drizzle:
       k_loop_typdiag_1: do k = kbot,ktop_typeDiag,kdir
          ...
          if (qr(i,k).ge.qsmall .and. nr(i,k).ge.nsmall) then ! FRED DEBUG excessive size sorting here also
